...
// executable. Only the extern "C" functions declared in gpugeno_cuda.h cross
...
// Complete definition of the opaque context declared in gpugeno_cuda.h. It
...
int flagstat_impl(struct gpugeno_cuda_context *context, const unsigned char *data,
    ...
    GPUGENO_CUDA_CHECK_AFTER_ENQUEUE(
        ...
        cudaEventRecord(context->kernel_start, context->stream));
    const cudaError_t launch_error = gpugeno_launch_flagstat(
        context->device_bytes, byte_count, context->device_span_starts, span_count,
        context->device_flagstat_results, context->device_flagstat_status, context->stream);
    if (launch_error != cudaSuccess) {
